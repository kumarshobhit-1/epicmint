...
function CreateWithAiPage() {
  ...
  // Set page title
  useEffect(() => {
    document.title = 'Create with AI - EpicMint';
  }, []);
  ...
  async function onMint(values: z.infer<typeof mintingSchema>) {
    ...
    try {
      const newNft = await addNft({
        ...
        category: 'story',
        tags: ['ai-generated'],
        price: 0.05, // Default price for AI-generated NFTs
        isListed: true, // Auto-list AI NFTs
      });
                ...
                {!isConnected ? (
                  ...
                  <Form {...mintingForm}>
                    <form onSubmit={mintingForm.handleSubmit(onMint)} className="space-y-4">
                      ...
                      />
                      <Button type="submit" className="w-full" size="lg" disabled={isMinting}>
                        {isMinting && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
                        {isMinting ? 'Minting...' : 'Mint This Creation'}
                      </Button>
                    </form>
                  </Form>
                )}
